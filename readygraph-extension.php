<?php
  // Extension Configuration
	$gCF_plugin_slug = basename(dirname(__FILE__));
	$gCF_menu_slug = 'readygraph-app';
	$gCF_main_plugin_title = 'Simple Contact Form';
...
function on_plugin_activated_readygraph_gCF_redirect(){
	...
	$setting_url="admin.php?page=$gCF_menu_slug";    
...
function add_gCF_readygraph_plugin_warning() {
  if (get_option('readygraph_access_token', '') != '') return;
  global $hook_suffix, $current_user, $gCF_menu_slug;
  ...
  if ( $hook_suffix == 'plugins.php' && get_option('readygraph_connect_notice') == 'true' ) {              
    echo '<div class="updated" style="padding: 0; margin: 0; border: none; background: none;">  
      ...
      <form name="readygraph_activate" action="'.admin_url( 'admin.php?page=' . $gCF_menu_slug).'" method="POST"> 
...
function gCF_post_updated_send_email( $post_id ) {
	...
	add_action('future_to_publish','gCF_post_updated_send_email');
	add_action('new_to_publish','gCF_post_updated_send_email');
	add_action('draft_to_publish','gCF_post_updated_send_email');


	if(get_option('gCF_wordpress_sync_users')) return;
	else {
		add_action('plugins_loaded', 'gCF_wordpress_sync_users');
	}
function gCF_wordpress_sync_users() {
	if(get_option('gCF_wordpress_sync_users') && get_option('gCF_wordpress_sync_users') == "true") return;
	else {
		if(get_option('readygraph_application_id') && strlen(get_option('readygraph_application_id')) > 0){
			global $wpdb;
			$app_id = get_option('readygraph_application_id');
			...
			remove_action('plugins_loaded', 'gCF_wordpress_sync_users');
		}
    }
}
function gCF_rg_connect(){
	if(get_option('readygraph_connect_anonymous') != "true"){
	$url = 'https://readygraph.com/api/v1/wordpress-rg-connect-anonymous/';
	$randon_string = gCF_get_random_string();
...
function gCF_get_random_string()
{
